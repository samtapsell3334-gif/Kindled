  ...
  return (
    <section aria-label="Example wishes" className="rounded-2xl border border-stone-200 bg-white p-4">
      <div className="flex items-center justify-between gap-2">
        ...
        <span className="shrink-0 rounded-full bg-stone-100 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide text-stone-500">
          Example wishes
        </span>
      </div>
      <p className="mt-0.5 text-[11px] text-stone-400">Seeded demo data, shown so you can see the finish line.</p>
          ...
          <div key={e.title} className="flex items-center gap-3 rounded-xl bg-stone-50 p-3">
            ...
            <div className="min-w-0">
              <p className="flex flex-wrap items-baseline gap-x-2 text-[13px] font-semibold text-stone-800">
                {e.title}
                <span className="text-[10px] font-bold uppercase tracking-wide text-emerald-600">{e.outcome}</span>
              </p>
              <p className="text-[12px] leading-snug text-stone-500">{e.line}</p>
            </div>
          </div>
        ))}
      </div>
